<!-- Search button -->
<div x-data="{ formCategoryOpen: false }" class="z-0">
    <!-- Button -->
    <button
        class="px-3 py-2 lg:gap-x-2 z-0 font-medium text-sm inline-flex items-center justify-center rounded-lg leading-5 transition bg-white border-2  border-gray-200 hover:border-gray-300  text-gray-400"
        @click.prevent="formCategoryOpen = true;if (formCategoryOpen) $nextTick(()=>{$refs.searchInput.focus()});"
        aria-controls="modal-create-category-vehicle">
    ...
    <!-- Modal backdrop -->
    <div class="fixed inset-0 bg-gray-900 bg-opacity-30 z-50 transition-opacity" x-show="formCategoryOpen"
    ...
    <!-- Modal dialog -->
    <div id="modal-create-category-vehicle"
        class="fixed inset-0 z-50 overflow-hidden flex items-center top-20 mb-4 justify-center px-4 sm:px-6"
        role="dialog" aria-modal="true" x-show="formCategoryOpen" x-transition:enter="transition ease-in-out duration-200"
        ...
        <div class="bg-white  border border-transparent  overflow-auto max-w-2xl w-full max-h-full rounded-lg shadow-lg"
            @click.outside="formCategoryOpen = false" @keydown.escape.window="formCategoryOpen = false">
            <!-- Search form -->
            <form class="border-b border-gray-200 ">
                <div class="relative">
                    <div class="bg-white p-4 rounded-lg w-full">
                        ...
                        <p class="text-gray-600 mb-6">Jangan khawatir, data ini masih bisa diubah apabila terdapat
                            kesalahan.
                        </p>
                        <div class="mb-4">
                            <input type="text" placeholder="Masukkan nama category"
                        ...
                        <div class="grid grid-cols-2 gap-x-3 w-full">
                            <button type="submit" @click.prevent="formCategoryOpen = false"
